...
def get_average(arr, mosaic_size):
    ...
    Мне пришлось сюда передавать mosaic_size, чтобы тест реализовать
    """
...
def draw_mosaic_pixel(color):
    ...
    doctest:
    Не получилось у меня сюда тесты прикрутить, сильно код менять приходится
    """
